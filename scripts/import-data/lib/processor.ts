...
export class EventProcessor extends ContentProcessor<ExternalEvent> {
  ...
  getSlug(event: ExternalEvent): string {
    ...
    const expectedSlug = truncateSlugAtWordBoundary(fullSlug);

    if (existingSlug) {
      const isAutoGenerated = existingSlug.startsWith(`${event.id}-`);
      if (isAutoGenerated && existingSlug !== expectedSlug) {
        this.renameSlug(meetupIdKey, existingSlug, expectedSlug);
      }
      return this.existingSlugsByMeetupId.get(meetupIdKey)!;
    }

    this.existingSlugsByMeetupId.set(meetupIdKey, expectedSlug);
    return expectedSlug;
  }

  private renameSlug(meetupIdKey: string, oldSlug: string, newSlug: string): void {
    const oldDir = path.join(config.paths.events, oldSlug);
    const newDir = path.join(config.paths.events, newSlug);

    if (existsSync(newDir)) {
      logger.warn(
        `Cannot rename "${oldSlug}" → "${newSlug}": target folder already exists. Keeping old slug.`,
      );
      return;
    }
